...
def detect_traffic_signals(maze_image):
	...
	x = list(np.unique(x_coordinates(xy_coordinate(maze_image, [255,0,0]))))
	y = list(np.unique(y_coordinates(xy_coordinate(maze_image, [255,0,0]))))	

	# get traffic_signal_node coordinates (image, red)
	traffic_cord = xy_coordinate(maze_image, [0,0,255])		
	...
	# get the x,y traffic_light coordinates
	x_t = []	
	y_t = []	
	for t_coords in traffic_cord:
		x_t.append(t_coords[0])
		y_t.append(t_coords[1])
	
	# store output in the form labels using coordinates & dictionary mapping
	l_x = []								
	l_y = []								
	...
	# combining l_x (x_label) & l_y (y_label)
	output = []
	for i in range(len(l_x)):
		output.append("".join(str(j) for j in (l_x[i]+l_y[i])))
