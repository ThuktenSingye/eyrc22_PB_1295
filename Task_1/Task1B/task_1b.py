...
def detect_Qr_details(image):
    ...
    for qrCodes in code:
        centerX = int(qrCodes[2][0] + qrCodes[2][3]/2)
        centerY = int(qrCodes[2][1] + qrCodes[2][2]/2)
        Qr_codes_details[(qrCodes[0]).decode()] = [centerX, centerY]
...
def detect_ArUco_details(image):
    ...
    for id in ids:
        arr = corners[i:(i+1)]
        ArUco_corners[int(id[0])]=arr[0][0]
        i+=1
        
    for ids, corner in ArUco_corners.items():
        angle = int(math.degrees(math.atan2(-(corner[1][1] - corner[0][1]), corner[1][0] - corner[0][0])))
        centerX = int((corner[0][0] + corner[2][0]) / 2.0)
        centerY = int((corner[0][1] + corner[2][1]) / 2.0)
        ArUco_details_dict[ids] = [[centerX, centerY], angle]
